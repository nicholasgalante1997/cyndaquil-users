import { dbService, CREATE_USER_INSERT_COMMAND, QUERY_USERS_TEMPLATE, QUERY_USER_TEMPLATE_BY_ID } from '../../db';
...
export class UserService implements IModel<UserDAO> {
    ...
    async create(item: UserDAO): Promise<UserDAO> {
        const entry = await dbService.query(CREATE_USER_INSERT_COMMAND, [item.user_id, item.email, item.salted_password]);
        console.log({ entry });
        return {
            email: '',
            salted_password: '',
