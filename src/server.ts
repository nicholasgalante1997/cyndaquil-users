...
app.get('/', (req, res) => {
  healthCheckHandlerWithMetrics.time(() => healthCheckHandler(req, res));
});
...
app.post('/users/create', validateAuthHeader, (req, res) => {
  createUserHandlerWithMetrics.monitor(
    // record metrics on if the op succeeds or fails
    () =>
      createUserHandlerWithMetrics.time(
        // record the duration of the op event
